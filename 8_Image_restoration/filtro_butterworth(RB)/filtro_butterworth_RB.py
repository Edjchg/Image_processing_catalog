...
"""
def f_butterworth_rb(nombre_imagen):
    ...
    for u in range(m):
        for v in range(n):
            ...
            F_B[u,v]=1/(1+(D_uv*W/(D_uv**2-D0**2))**(2*orden))#
...
B, M, RC  = f_butterworth_rb(img)
plt.figure()
#Imagen original
plt.subplot(221),plt.title("Imagen original")
plt.imshow(A, cmap='gray')
#Mascara 2
plt.subplot(222),plt.title("Mascara filtro butterworth rechaza banda")
