...
"""
def f_butterworth_alto(nombre_imagen):
    ...
    for u in range(m):
        for v in range(n):
            ...
            F_B[u,v]=1/(1+(D0/(1+D_uv)**(2*orden)))#Suma de 1 para evitar divison por cero
...
B, M, RC  = f_butterworth_alto(img)
plt.figure()
#Imagen original
plt.subplot(221),plt.title("Imagen original")
plt.imshow(A, cmap='gray')
#Mascara 2
plt.subplot(222),plt.title("Mascara filtro butterworth paso-alto")
